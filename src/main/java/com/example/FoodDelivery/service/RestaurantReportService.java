...
public class RestaurantReportService {
    ...
    public FullReportDTO getFullReport(Long restaurantId, Instant startDate, Instant endDate)
        ...
        BigDecimal netRevenue = orders.stream()
                ...
                .map(Optional::get)
                .map(s -> s.getRestaurantNetEarning() != null ? s.getRestaurantNetEarning() : BigDecimal.ZERO)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    public List<RevenueReportItemDTO> getRevenueReport(Long restaurantId, Instant startDate, Instant endDate)
        ...
        for (LocalDate date = start; !date.isAfter(end); date = date.plusDays(1)) {
            ...
            for (Order order : dayOrders) {
                foodRevenue = foodRevenue.add(order.getSubtotal() != null ? order.getSubtotal() : BigDecimal.ZERO);
                ...
                if (summary != null) {
                    commissionAmount = commissionAmount.add(
                            summary.getRestaurantCommissionAmount() != null ? summary.getRestaurantCommissionAmount()
                                    : BigDecimal.ZERO);
                    netRevenue = netRevenue
                            .add(summary.getRestaurantNetEarning() != null ? summary.getRestaurantNetEarning()
                                    : BigDecimal.ZERO);
                }
            }
    ...
    private OrderReportItemDTO convertToOrderReportItem(Order order) {
        ...
        if (order.getCustomer() != null) {
            ...
            customerPhone = order.getCustomer().getPhoneNumber() != null ? order.getCustomer().getPhoneNumber() : "N/A";
        }
            ...
            if (order.getOrderItems() != null) {
                for (OrderItem item : order.getOrderItems()) {
                    if (item.getDish() != null && item.getDish().getId().equals(dish.getId())) {
                        ...
                        BigDecimal price = item.getPriceAtPurchase() != null ? item.getPriceAtPurchase()
                                : dish.getPrice();
                        if (price != null) {
        ...
        return categoryMap.values().stream()
                .peek(c -> {
                    if (totalRevenue.compareTo(BigDecimal.ZERO) > 0) {
                        c.setPercentOfTotal(c.getTotalRevenue().multiply(BigDecimal.valueOf(100))
